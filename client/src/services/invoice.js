  ...
  return items
    .map(task => {
      ...
      const [subTotal, hours] = filteredTimeslips.reduce(
        ([t, h], timeslip) => [
          t + timeslip.hourly_rate * timeslip.hours,
          h + timeslip.hours,
        ],
        [0, 0]
      );
...
export const getDisplayItems = (invoice, rate, tasks = []) => {
  const taskIds = invoice.tasks || [];
  const timeslipIds = invoice.timeslips || [];
  const displayTimeslips = tasks.reduce(
    (all, task) =>
      task.billing_type === 'TIME' ? all.concat(task.timeslips) : all,
    []
  );

  const items =
    invoice.group_by === 'tasks'
